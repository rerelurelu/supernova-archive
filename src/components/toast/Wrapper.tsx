...
import type { State, ToasterProps } from '~/types';
import { Theme } from '~/types';
import { DEFAULT_WRAPPER_OPTIONS, GAP, TOAST_WIDTH, createOptionsObject } from '~/utils';
...
export const Toaster = component$<ToasterProps>((props) => {
  ...
  const keyShortcut = opts.hotkey.join(' + ').replace(/Key/g, '').replace(/Digit/g, '');
  ...
  useVisibleTask$(() => {
    return toastState.subscribe((toast) => {
      if (toast.dismiss) {
        state.toasts = state.toasts.map((t) => (t.id === toast.id ? { ...t, dismiss: true } : t));
        return;
      }
  ...
  useOnDocument(
    ...
    $((event: any) => {
      ...
      const isHotkeyPressed = opts.hotkey.every((key) => (event as any)[key] || event.code === key);
